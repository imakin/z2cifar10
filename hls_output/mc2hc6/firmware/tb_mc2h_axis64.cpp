...
int main(int argc, char **argv) {
    try {
        ...
        if (s_out.empty()) {
            std::fprintf(stderr, "ERROR: No output produced\n");
            return 1;
        }
        axis64_t o = s_out.read();
        ap_uint<64> od = o.data;
        int16_t y0 = (int16_t)od.range(15,0);
        int16_t y1 = (int16_t)od.range(31,16);
        std::printf("Output Q6.10: y0=%d, y1=%d (TLAST=%u)\n", (int)y0, (int)y1, (unsigned)o.last);
        return 0;
